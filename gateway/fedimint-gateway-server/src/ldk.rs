...
use fedimint_gateway_common::{
    ...
    CreateInvoiceRequest, CreateInvoiceResponse, GetBalancesResponse, GetInvoiceRequest,
    GetInvoiceResponse, GetLnOnchainAddressResponse, GetNodeInfoResponse, GetRouteHintsResponse,
    InterceptPaymentRequest, InterceptPaymentResponse, InvoiceDescription, LightningRpcError,
    ListChannelsResponse, ListTransactionsResponse, OpenChannelRequest, OpenChannelResponse,
    PayInvoiceResponse, PaymentAction, Preimage, RouteHtlcStream, SendOnchainRequest,
    SendOnchainResponse,
};
...
pub struct GatewayLdkClient {
    ...
    /// The HTLC stream, until it is taken by calling `route_htlcs`.
    ...
    /// This helps ensure that the function is idempotent.
    ...
    /// simultaneous calls with the same offer to execute in parallel. This
    /// helps ensure that the function is idempotent.
...
impl GatewayLdkClient {
    ...
    pub async fn routehints(
        ...
        // LDK uses LNv2 payment receives where the gateway's lightning node
        // generates invoices and inserts proper route hints on its own.
    ...
    pub fn route_htlcs(&mut self) -> Result<RouteHtlcStream<'static>, LightningRpcError> {
        match self.htlc_stream_receiver_or.take() {
    ...
    /// `num_route_hints`.
    pub async fn parsed_route_hints(
        &self,
        num_route_hints: u32,
    ) -> Vec<fedimint_gateway_common::RouteHint> {
    ...
    /// node.
    pub async fn parsed_node_info(&self) -> fedimint_gateway_common::LightningInfo {
        ...
        {
            return fedimint_gateway_common::LightningInfo::Connected {
        ...
        fedimint_gateway_common::LightningInfo::NotConnected
    }
    ...
    /// invoices.
    pub fn supports_private_payments(&self) -> bool {
        false
    }
}
