...
/// database. Exposed separately so the LDK event loop can span the gateway
/// and client namespaces with one write transaction (via
/// `DatabaseTransaction::to_ref_with_prefix`).
pub fn client_db_prefix(federation_id: &FederationId) -> Vec<u8> {
...
pub fn get_client_database(db: &Database, federation_id: &FederationId) -> Database {
    db.with_prefix(client_db_prefix(federation_id))
}
...
pub struct IncomingContractRow {
    ...
    pub invoice: LightningInvoice,
    /// The invoice amount — the contract amount plus the gateway's receive
    /// fee. This is what an inbound HTLC must pay and the claimable amount
    /// asserted when settling it on the lightning node.
    pub amount: Amount,
}
