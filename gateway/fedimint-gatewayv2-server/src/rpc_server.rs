...
fn lnv2_routes() -> Router {
    ...
    let router = register_post_handler(ROUTING_INFO_ENDPOINT, routing_info_v2, false, router);
    let router = register_post_handler(SEND_PAYMENT_ENDPOINT, pay_bolt11_invoice_v2, false, router);
...
fn routes(gateway: Arc<Gateway>, task_group: TaskGroup) -> Router {
    ...
    let mut public_routes =
        register_post_handler(RECEIVE_ECASH_ENDPOINT, receive_ecash, false, Router::new());
