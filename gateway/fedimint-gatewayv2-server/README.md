...
gateway or any other client) into your balance:

```bash
gatewaydv2-cli federation module mint receive <federation-id> <ecash>
```
...
| `RECEIVE_FEE_PPM`             | no       | `0`              | Proportional gateway fee cut on incoming payments |
| `ROUTING_FEE_BASE_MSAT`       | no       | `10000`          | Base LN routing fee budget, enforced as LDK's max routing fee on external sends |
| `ROUTING_FEE_PPM`             | no       | `3000`           | Proportional LN routing fee budget          |
| `FM_PROBE_TOP_NODE_COUNT`     | no       | `100`            | How many top-connected nodes the background prober cycles through |
| `FM_PROBE_DIVERSITY_PENALTY_MSAT` | no   | `250`            | Scorer penalty on recently probed channels so probes explore new routes |
| `FM_PROBE_MAX_LOCKED_MSAT`    | no       | `500000000`      | Cap on liquidity locked in in-flight probes |
