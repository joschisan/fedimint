...
pub trait IDatabaseTransaction: MaybeSend + IWriteDatabaseTransactionOps + fmt::Debug {
    ...
    /// Strip prefix wrappers to get the underlying global transaction.
    ///
    /// Meant to be called only by core internals to let module code reach
    /// the global dbtx.
    #[doc(hidden)]
    fn global_dbtx(&mut self) -> &mut dyn IDatabaseTransaction;
}
...
impl<'tx, Cap> WriteDatabaseTransaction<'tx, Cap> {
    ...
    /// Strip module prefix wrappers to access the underlying global dbtx.
    pub fn global_dbtx<'a>(&'a mut self) -> WriteDatabaseTransaction<'a, Cap>
    where
        'tx: 'a,
    {
        let decoders = self.decoders.clone();
        WriteDatabaseTransaction {
            tx: Box::new(self.tx.global_dbtx()),
            decoders,
            commit_tracker: match self.commit_tracker {
                MaybeRef::Owned(ref mut o) => MaybeRef::Borrowed(o),
                MaybeRef::Borrowed(ref mut b) => MaybeRef::Borrowed(b),
...
pub struct DbMigrationFnContext<'tx, C> {
    dbtx: WriteDatabaseTransaction<'tx, NonCommittable>,
...
impl<'tx, C> DbMigrationFnContext<'tx, C> {
    ...
    #[doc(hidden)]
    pub fn split_dbtx_ctx<'s>(
        &'s mut self,
    ) -> (&'s mut WriteDatabaseTransaction<'tx, NonCommittable>, &'s C) {
        let Self { dbtx, ctx, .. } = self;

        (dbtx, ctx)
    }

    pub fn dbtx(&'_ mut self) -> WriteDatabaseTransaction<'_, NonCommittable> {
...
pub async fn apply_migrations_dbtx<C>(
    global_dbtx: &mut WriteDatabaseTransaction<'_, NonCommittable>,
    ctx: C,
    kind: String,
    migrations: BTreeMap<DatabaseVersion, DbMigrationFn<C>>,
    module_instance_id: Option<ModuleInstanceId>,
...
async fn remove_current_db_version_if_exists(
    version_dbtx: &mut WriteDatabaseTransaction<'_, NonCommittable>,
    is_new_db: bool,
    target_db_version: DatabaseVersion,
) -> DatabaseVersion {
...
mod test_utils {
    ...
    pub async fn verify_find_by_range(db: Database) {
        ...
        {
            let mut module_dbtx = dbtx.to_ref_with_prefix_module_id(2);
            module_dbtx
        ...
        let mut module_dbtx = dbtx.with_prefix_module_id(2);
