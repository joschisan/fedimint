...
#[derive(Clone, Debug, Encodable, PartialEq, Eq, Hash)]
...
#[derive(Clone, Debug, Decodable, Encodable, PartialEq, Eq, Hash)]
enum OOBNotesPart {
...
impl MintClientModule {
    ...
    ///
    /// The amount will be rounded up to the nearest multiple of 512 msat.
    ///
    ...
    ///    completes.
    ///
    /// **Safe cancellation**: If the method enters online mode and is cancelled
    /// (e.g., the future is dropped), any reissued notes will automatically be
    /// returned to the wallet. No funds are lost.
    ///
    /// # Arguments
    /// * `amount` - The amount to send (will be rounded up to next 512 msat
    ///   multiple)
    /// * `extra_meta` - Additional metadata to store with the operation
    ///
    /// # Returns
    /// Returns the `OOBNotes` that can be sent out-of-band.
