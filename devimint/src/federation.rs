...
use fedimintd::devimint::{
    ADD_SETUP_CODE_ENDPOINT, INIT_SETUP_ENDPOINT, InitSetupRequest, START_DKG_ENDPOINT,
};
...
pub async fn run_devimint_setup_procedure(endpoints: BTreeMap<PeerId, String>) -> Result<()> {
    ...
    for (peer, endpoint) in &endpoints {
        ...
        let setup_code = poll_simple("init-setup", || async {
            client
                .post(format!("http://{endpoint}/{INIT_SETUP_ENDPOINT}"))
                .json(&request)
                .send()
                .await?
                .text()
    ...
    for (peer, setup_code) in setup_codes {
        for (p, endpoint) in &endpoints {
            if p != peer {
                client
                    .post(format!("http://{endpoint}/{ADD_SETUP_CODE_ENDPOINT}"))
                    .json(&setup_code)
                    .send()
                    .await?;
            }
    ...
    for endpoint in endpoints.values() {
        client
            .post(format!("http://{endpoint}/{START_DKG_ENDPOINT}"))
            .send()
            .await?;
    }
