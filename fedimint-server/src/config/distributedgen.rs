...
use fedimint_core::config::{DkgMessage, DkgPeerMsg};
...
pub async fn run_dkg(
    ...
    connections
        .send(
            Recipient::Everyone,
            DkgPeerMsg::DistributedGen(dkg.initial_message()),
        )
        .await;
        ...
        for peer in num_peers.peer_ids().filter(|p| *p != identity) {
            ...
            match dkg.step(peer, message)? {
                DkgStep::Broadcast(message) => {
                    connections
                        .send(Recipient::Everyone, DkgPeerMsg::DistributedGen(message))
                        .await;
                }
                DkgStep::Messages(messages) => {
                    for (peer, message) in messages {
                        connections
                            .send(Recipient::Peer(peer), DkgPeerMsg::DistributedGen(message))
                            .await;
                    }
                }
...
mod tests {
    ...
    use crate::config::distributedgen::{eval_poly_g1, eval_poly_g2, g1, g2, Dkg, DkgStep};
