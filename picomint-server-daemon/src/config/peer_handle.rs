...
pub struct PeerHandle<'a> {
    // TODO: this whole type should be a part of a `picomint-server` and fields here inaccessible
    // to outside crates, but until `ServerModule` is not in `picomint-server` this is impossible
    #[doc(hidden)]
